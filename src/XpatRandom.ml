...
(* a) *)
let init seed =
  let rec loop l x prev1 prev2 =
    let y = (prev1 + 21) mod 55 in
    if x = 55 then l
    else (
      if x = 0 then loop (l @ (0, seed) :: []) (x + 1) 0 prev1
      else if x = 1 then loop (l @ (y, 1) :: []) (x + 1) y prev1
      else 
        let (_, a) = List.nth l (x - 2) in
        let (_, b) = List.nth l (x - 1) in
        if a < b then loop (l @ (y, a - b + randmax) :: []) (x + 1) y prev1
        else loop (l @ (y, a - b) :: []) (x + 1) y prev1
    )
  in loop [] 0 0 0

(* b) *)
let sort l = List.sort (fun (k1, v1) (k2, v2) -> compare k1 k2) l
...
let split_list_to_fifo l =
  let l' = sort l in
  let f1, f2 = split_at 24 [] l' in
  let (_, f1_init), (_, f2_init) = List.split f2, List.split f1 in (Fifo.of_list f1_init, Fifo.of_list f2_init)

(* c) *)
let tirage f1 f2 =
  let (n1, f1') = Fifo.pop f1 in
  let (n2, f2') = Fifo.pop f2 in
  let d = if (n1 < n2) then (n1 - n2 + randmax) else (n1 - n2) in
  let f1'', f2'' = Fifo.push n2 f1', Fifo.push d f2' in (f1'', f2'', d)

(* d) *)
let ignore_165 f1 f2 = 
  let rec loop n f1' f2' = 
    if n = 0 then (f1', f2')
    else (let f1_t, f2_t, _ = tirage f1' f2' in loop (n - 1) f1_t f2_t)
  in loop 165 f1 f2

(* e) *)
let tirage_52 f1 f2 =
  let rec loop n f1' f2' result = 
    if n = 0 then result 
    else (let f1_t, f2_t, d = tirage f1' f2' in loop (n - 1) f1_t f2_t (d :: result))
  in List.rev (loop 52 f1 f2 [])

let reduced_52 f =
  let rec loop n f' result =
    if n = 0 then result
    else
      match f' with
      | [] -> result
      | h :: t -> loop (n - 1) t (reduce h n :: result)
  in List.rev (loop 52 f [])

let remove_at n l =
  List.filteri (fun i x -> i <> n ) l

let init_list_52 =
  let rec loop n l =
    if n = 0 then (n :: l) else loop (n - 1) (n :: l)
  in loop 51 []

let swap l =
  let list_52 = init_list_52 in
  let rec loop reduced_52 l' result =
    if l' = [] then result
    else (
      let n, r = Fifo.pop reduced_52 in
      let len = List.length l' in
      let x = if n >= len then List.nth l' (len - 1) else List.nth l' n in
      if n >= len then loop r (remove_at (len - 1) l') (x :: result) else loop r (remove_at n l') (x :: result)
    )
  in loop l list_52 []
...
let shuffle n =
  ...
  let l = init n in 
  let f1_init, f2_init = split_list_to_fifo l in 
  let f1_165, f2_165 = ignore_165 f1_init f2_init in
  let tirage_52 = tirage_52 f1_165 f2_165 in
  let reduced_52 = reduced_52 tirage_52 in
  swap (Fifo.of_list (List.rev reduced_52))
